...
class AppModel extends EventEmitter {
  ...
  initHandlers() {
    ...
    document.body.addEventListener('click', elem => {
      ...
      if (
        (deviceType === 'hybrid' || deviceType === 'touchOnly') &&
  ...
  setDefaultConfigFlyby(config) {
    if (config['genplan']) {
      ...
      // const type = 'flyby';
      // const flyby = Object.keys(configFlyby)[3];
      // const side = 'outside';
      // this.defaultFlybySettings = {
      //   type,
      //   flyby,
  ...
  updateFsm(data, nextHistory = true, sliderData, cb) {
    ...
    if (nextHistory) {
      ...
      dispatchTrigger('updateFsm', { ...data, getFlat: this.getFlat, getFloor: this.getFloor });
  ...
  getCurrency() {
    return Object.keys(this.flatList)[0].currency || 'USD';
  }
  ...
  async asyncGetConstructionProgress() {
    ...
    try {
      ...
      if (isObject(docsData)) {
        this.constructionProgressData = { ...docsData };
      }
    } catch (err) {}
  }
}

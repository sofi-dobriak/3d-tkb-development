...
class FilterView extends EventEmitter {
  ...
  setFilterStatusClassToBody() {
    document.body.classList.toggle('js-s3d-filter-open', document.querySelector('.js-s3d-filter').classList.contains('s3d-open-filter'));
  }

  setFilterStatusToButtons() {
    document.querySelectorAll('.js-s3d-ctr__filter').forEach((el) => {
      el.classList.toggle('active', document.querySelector('.js-s3d-filter').classList.contains('s3d-open-filter'));
    });
  }
  ...
  toggle() {
    ...
    if ($filter.classList.contains('s3d-open-filter')) {
      gsap.timeline()
        .to($filter, {  duration: 0.3, x: '-100%', ease: 'Power1.easeIn'})
        .add(() => {

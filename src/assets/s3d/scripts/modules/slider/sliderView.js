    ...
    Object.entries(flybyAndBuildNamesMap).forEach(([build_name, flybyName]) => {
      ...
      document
        ...
        .forEach(el => {
          ...
          const finishDate = get(
            this._model,
            ['flyby_finish_dates', `${flyby}-${side}`],
            undefined,
          );
          ...
          const $tooltip = isMobile()
            ? MobileSvgFlybyTooltip({
                title: this._model.i18n.t(`ctr.nav.flyby_${flyby}_${side}`),
                x,
                y,
                flyby,
                side,
            ...
            : SvgFlybyTooltip({
                ...
                rightTitle1:
                  this._model.show_prices === true
                    ? `<span>${this._model.i18n.t('from')} ${this._model.i18n.t(
                        'currency_label',
                      )}</span> <div style="display: inline"> ${minPrice}</div>`
                    : '',
                rightTitle2:
                  this._model.show_prices === true

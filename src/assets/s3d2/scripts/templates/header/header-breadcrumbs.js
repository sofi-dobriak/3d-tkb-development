import { find, groupBy, isArray, isObject } from 'lodash';
import {
  $breadCrumbsFlatItem,
  $breadcrumbsDropdown,
  $breadcrumbsFlybyItem,
  $breadcrumbsItem,
} from '../templates/header/$breadcrumbs';
...
const schema = {
  ...
  flyby_4_inside: [
    { type: 'earth' },
    { type: 'map' },
    { type: 'flyby_4_outside' },
    { type: 'flyby_4_inside' },
  ],
  flat: [
    { type: 'earth' },
    { type: 'map' },
    { type: FIND_LAST_OUTSIDE },
    { type: FIND_LAST_INSIDE },
    { type: 'flat', id: FIND_FLAT_ID },
  ],
  ...
  constructor(props) {
    ...
    this.trigger(data => {
      ...
      if (type === 'flyby' && schema[flybyBreadcrumbsKey]) {
        return this.render(
          schema[flybyBreadcrumbsKey].map(item => {
      ...
      if (type === 'flat') {
        const flatBreadcrumbs = schema[type].map(breadItem => {
          switch (breadItem.type) {
            case FIND_LAST_OUTSIDE:
              return findLastOutside(this.history.histories);
            case FIND_LAST_INSIDE:
              return findLastInside(this.history.histories);
            default:
              return {
                ...breadItem,

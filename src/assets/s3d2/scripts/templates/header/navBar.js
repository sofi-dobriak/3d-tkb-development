  ...
  return Object.entries(config.flyby).reduce((acc, [flyby, flybyData]) => {
    Object.entries(flybyData).map(([side, sideData]) => {
      acc.push({
        ...
        className: 'js-s3d-nav__btn',
        title: i18n.t(`ctr.nav.flyby_${flyby}_${side}`),
        active: e.detail.type === 'flyby' && e.detail.flyby === flyby && e.detail.side === side,
      });
    });
    return acc;
  }, []);
...
export default function navBar(i18n, { logo }) {
  ...
  document.body.addEventListener('click', e => {
    ...
    if (!target) return;
    document.querySelector('.s3d2-header__hide-block').classList.toggle('active');
    const block = document.querySelector('.s3d2-header__hide-block');
    gsap
      .timeline()
      .to(block, {
        autoAlpha: block.classList.contains('active') ? 0 : 1,
        duration: 0.13,
      })
      .to(block, {
        display: block.classList.contains('active') ? 'none' : '',
      });
  });
    ...
    document
      ...
      .insertAdjacentHTML(
        ...
        Dropdown(
          ...
          e.detail.type === 'flyby' ? 'highlighted' : '',
        ),
      );
  ...
  console.log('dataForFlybyDropdown', dataForFlybyDropdown);
  ...
  return `
    <header class="s3d2-header">
      ...
      <button class="s3d2-header__back-button" onclick="window.history.back()">
        ${s3d2spriteIcon('Back', 's3d2-header__back-button-icon')}
        <span>${i18n.t('ctr.nav.back')}</span>
      </button>
      <div class="s3d2-header__hide-block">
        <button class="s3d2-header__back-button js-s3d-nav__btn" type="button" data-type="plannings" data-dont-make-me-active data-s3d2-header-plannings>
          ${s3d2spriteIcon('Card', 's3d2-header__back-button-icon')}
        </button>
        ${
          config.genplan
        ...
        ${s3d2spriteIcon('Next', 's3d2-header__between-icon')}
